...
# os.chdir("rpa_basic") # change directory, rpa_basic 으로 작업 공간 이동
# print(os.getcwd())
# os.chdir("..") # 부모(상위) 폴더로 이동 C:\cripto\rpa_basic -> C:\cripto
# print(os.getcwd())
# os.chdir("../..") # 조부모 폴더로 이동 ../../../../ ,,,
# print(os.getcwd())
...
# 파일 경로 만들기  현재 절대 경로에 C:\cripto\rpa_basic 내가 추가할 경로를 추가한다
# file_path = os.path.join(os.getcwd(), "my_file.txt") # 절대 경로 생성  # os.getcwd() : 현재 작업 경로, my_file.txt 
# print(file_path)

# 파일 경로에서 폴더 정보 가져오기  , 지금 어떤 폴더에 파일이 있는데, 똑같은 경로에 다른 파일도 만들어야 할때.
...
# 파일의 생성 날짜
# file_path = "rpa_basic/2_desktop/11.file system copy.py"
# ctime = os.path.getctime(file_path) # create 타임
# print(ctime) #이상한 숫자로 뜬다 포맷 변경 필요
# # # 날짜 정보를 strftime 을 통해서 연월일 시분초 형태로 출력
# print(datetime.datetime.fromtimestamp(ctime).strftime("%Y%m%d %H:%M:%S"))

# # 파일의 수정 날짜
# mtime = os.path.getmtime(file_path)  # modified time
# print(datetime.datetime.fromtimestamp(mtime).strftime("%Y%m%d %H:%M:%S"))

# # 파일의 마지막 접근 날짜
# atime = os.path.getatime(file_path) # access time
...
# result = os.walk("rpa_basic") # rpa_basic 폴더 밑에 있는 모든 폴더, 파일 목록 가져오기
# result = os.walk(".") # 현재 폴더 기준
# print(result)  # <generator object walk at 0x033D5B18> 이렇게 만 뜬다. -> 반복문 사용 필요
...
#     if name in files:  #파일명이 File 리스트안에 있으면 result에 더해준다
#         result.append(os.path.join(root, name))
# print(result)  # ['.\\rpa_basic\\2_desktop\\11_file_system.py']

# 만약 폴더 내에서 특정 패턴을 가진 파일들을 찾으려면?
# *.xlsx, *.txt, 자동화*.png
# import fnmatch #파일 네임 일치하는지 여부 확인
# pattern = "*.py" # .py 로 끝나는 모든 파일    # pattern 에 들어갈 이름 정리 *.png , check*.png 
# result = []
# for root, dirs, files in os.walk("."):
#     # [a.txt, b.txt, c.txt, 11_file_system.py, ...]
#     for name in files:
...
# 폴더 만들기
# os.mkdir("new_folder") # 현재 경로 기준으로 폴더 생성   make directory
