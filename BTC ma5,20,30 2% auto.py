...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(minutes=10) and open_price*0.996 > current_price :
            ...
            if target_price <= current_price and ma5 < current_price and ma20 < current_price and ma30 < current_price:
                ...
                if target_percent  <= 0.02:
                    if btc < 0.0001:
                        buy_result = upbit.buy_market_order("KRW-BTC", krw*(0.99))
                elif 0.02 < target_percent:
                    if btc < 0.0001:
                        buy_result = upbit.buy_market_order("KRW-BTC", krw*(2/target_percent/100))                  
