...
# 적용 필요
# 다이버전스 룰로 15m, 1h, 4h 봉에서 추매 하는 로직 만들기

# 상기 코드는 9시마다 일,월,화,수에만 토요일 06:00 가격 기준으로 롱,숏을 선물 진입하는 코드입니다. 
# 여기에 추가적으로다른 조건의 매매도 추가하려고 하는데 가능할까요? 조건은 1시간 봉기준 직전 15캔들의 종가 기준 rsi를 조회합니다. 
# 그래서 최근 15캔들중 rsi가 제일 낮은 low rsi 캔들위치를 찾고, 바로 직전 캔들의 rsi는 low rsi보다 높으나, 종가는 low rsi 캔들보다 낮을때 시장가로 진입하려고 합니다.
# 즉 상승 다이버전스라고 볼수 있겠죠? 진입 비중은 계좌 달러의 0.5배로 시장가로 들어가고 진입 레버리지는 5배. 
# 진입 후 직전 캔들 종가값의 +1% 값에 T.P (take profit)걸어두기. 해당 조건으로 만들어주시겠어요?
