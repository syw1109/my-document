...
def get_confirmed_candles_with_rsi(
    symbol,
    timeframe,
    count=100,
    rsi_length=14
...
#12============거래량 장대봉 추세 전략=======
# 전역 변수 추가
pending_volume_breakout_signal = {}
...
def analyze_volume_breakout_strategy(
    ...
    return {
        ...
        "signal_candle_index": base_idx,  # ✅ 추가: 기준캔들 인덱스
        "signal_candle_close": base_close,  # ✅ 추가: 기준캔들 종가
    }
...
def trade_volume_breakout_strategy(
    ...
    if timeframe == '5m' and now - last_volume_breakout_5m < 2700:
        ...
        print(f"[{symbol} VOLUME_BREAKOUT 5m] 최근 {minutes_ago:.1f}분 전에 5 분봉 매수됨 (45 분 내 중복매수 금지)")
        return
    
    # ✅ 15m 전용 쿨다운 (45 분)
    if timeframe == '15m' and now - last_volume_breakout_15m < 2700:
    ...
    # ✅ 1h 전용 쿨다운 (180 분)
    if timeframe == '1h' and now - last_volume_breakout_1h < 10800:
    ...
    if sig and sig["signal"]:
        if key not in pending_volume_breakout_signal:
            pending_volume_breakout_signal[key] = {
                "signal_info": sig,
                "signal_time": now,
                "signal_candle_index": sig["signal_candle_index"],
                "signal_candle_close": sig["signal_candle_close"],
                "base_high": sig["base_high"],
                "base_low": sig["base_low"],
                "is_bullish_base": sig["is_bullish_base"],
                "is_yangbong": sig["is_yangbong"],
            }
            print(f"[{symbol} VOLUME_BREAKOUT] 기준캔들 시그널 발생 → 30 봉 동안 진입 조건 모니터링 (timeframe={timeframe})")
        # ✅ 기준캔들 시그널 발생 시 바로 진입하지 않음 (모니터링 시작)
        return
    
    # 2) 대기 중인 신호가 있으면, 진입 조건 확인
    if key not in pending_volume_breakout_signal:
        return
    
    pending = pending_volume_breakout_signal[key]
    ...
    base_high = pending["base_high"]
    base_low = pending["base_low"]
    is_bullish_base = pending["is_bullish_base"]
    is_yangbong = pending["is_yangbong"]
    
    # 기준캔들 이후 30 봉 초과 확인
    current_candle_index = len(df) - 1
    if current_candle_index - signal_candle_index > 30:
        print(f"[{symbol} VOLUME_BREAKOUT] 기준캔들 이후 30 봉 초과 → 대기 상태 초기화 (timeframe={timeframe})")
        del pending_volume_breakout_signal[key]
        return
    
    # 3) 진입 조건 확인 (기준캔들 이후 1~30 봉)
    monitor_start = signal_candle_index + 1
    monitor_end = current_candle_index
    ...
    for i in range(monitor_start, monitor_end + 1):
        ...
        if is_yangbong:
            if is_bullish_base:
                # 강세: -0.45%
                drop_threshold = signal_candle_close * (1 - entry_drop_pct_bullish)
            else:
                # 약세: -0.8%
                drop_threshold = signal_candle_close * (1 - entry_drop_pct_bearish)
            
            if monitor_close <= drop_threshold:
                entry_idx = i
                entry_price = monitor_open
                
                # TP: 기준캔들 윗꼬리 변동성 1% 이상이면 (H+C)/2*1.002, 아니면 base_high*1.002
                upper_shadow = base_high - signal_candle_close
                upper_shadow_volatility = upper_shadow / signal_candle_close
                if is_yangbong and upper_shadow_volatility >= 0.01:
                    tp_price = ((base_high + signal_candle_close) / 2) * 1.002
                else:
                    tp_price = base_high * 1.002
        ...
        # 음봉 숏 전략
            ...
            if is_bullish_base:
                # 강세: +0.45%
                rise_threshold = signal_candle_close * (1 + entry_rise_pct_bullish)
            else:
                # 약세: +0.8%
                rise_threshold = signal_candle_close * (1 + entry_rise_pct_bearish)
            ...
            if monitor_close >= rise_threshold:
                ...
                # TP: 기준캔들 아래꼬리 변동성 1% 이상이면 (L+C)/2*0.998, 아니면 base_low*0.998
                lower_shadow = signal_candle_close - base_low
                lower_shadow_volatility = lower_shadow / signal_candle_close
                if not is_yangbong and lower_shadow_volatility >= 0.01:
                    tp_price = ((base_low + signal_candle_close) / 2) * 0.998
                else:
                    tp_price = base_low * 0.998
    ...
    if amount <= 0:
        print(f"[{symbol} VOLUME_BREAKOUT] 주문 수량이 0 이라서 중단")
        del pending_volume_breakout_signal[key]
        return
...
def analyze_volume_spike_drop(
    ...
    - range_volatility 는 만족하지만 거래량 조건 불만족일 때
      아래 두 가지 중 하나를 만족하면 2 배 또는 1.5 배만 요구
    ...
    - [-9:] 구간 (직전봉 포함 9 개봉) 에서
      * 변동성 >= 0.5% 인 음봉 >= 2
      * 변동성 >= 0.35% 인 음봉 >= 3
      * 그중 꼬리가 몸통의 1/3 이상인 캔들 >= 2
    → 직전 3 개봉 [-4:-1] 거래량 평균의 1.5 배 이상이면 OK

    예외 조건 3, 4, 5 (롱/숏 공통):
    - 공통 조건 1:
      * 롱: 직전 12 개봉 중 (직전 11 개봉 low)*1.001 < 직전봉 종가
      * 숏: 직전 12 개봉 중 (직전 11 개봉 high)*0.999 > 직전봉 종가
    - 공통 조건 2: 캔들 변동성 0.1% 이상인데 꼬리가 0.4% 이상
      * 꼬리 2 개 또는 0.4% 캔들 1 개로 취급

    - 진입 예외 조건 3: 캔들 변동성 0.3% 이상 3 개, 0.4% 이상 1 개 + 꼬리 2 개 이상
    - 진입 예외 조건 4: 0.2% 3 개 이상, 0.5% 2 개 이상 + 꼬리 2 개 이상
    - 진입 예외 조건 5: 0.5% 3 개이상 + 0.4% 4 개이상 이면 꼬리 1 개로 진입 가능
    ...
    - 공통 조건 1:
      ...
      * 숏: 직전 12 개봉 중 (직전 11 개봉 high)*0.999 > 직전봉 종가
    - 공통 조건 2: 캔들 변동성 0.1% 이상인데 꼬리가 0.4% 이상
      * 꼬리 2 개, 0.4% 캔들 1 개 취급
      * 추가적으로 0.15% 3 개 이상이면 1/3 꼬리 1 개, 0.4% 봉 1 개로 취급

    - 15 분 예외 1: 0.4% 3 개 이상, 꼬리 2 개 이상
    - 15 분 예외 2: 0.15% 4 개 이상, 0.3% 2 개 이상, 꼬리 2 개 이상
    - 꼬리는 각 조건의 변동성 기준이 되는 캔들에 대해서만 적용
    """
    ...
    # 구간 정의
    last_8_ex_prev = df.iloc[-9:-1]  # 직전봉의 직전 8 개봉
    last_4_ex_prev = df.iloc[-5:-1]  # 직전봉의 직전 4 개봉
    last_3_ex_prev = df.iloc[-4:-1]  # 직전봉의 직전 3 개봉
    last_9_incl_prev = df.iloc[-9:]  # 직전봉 포함 9 개봉
    last_12_incl_prev = df.iloc[-12:]  # 직전봉 포함 12 개봉
    last_11_ex_prev = df.iloc[-12:-1]  # 직전봉의 직전 11 개봉

    # -------------------------------------------------
    # 1) range_volatility 계산 (직전봉의 직전 8 개봉)
    # -------------------------------------------------
    range_high = last_8_ex_prev['close'].max()
    range_low = last_8_ex_prev['close'].min()
    ...
    # -------------------------------------------------
    # 4) 롱일 때만 예외 조건 처리 (기존 exc1, exc2)
    # -------------------------------------------------
    cond_exc1 = False
    cond_exc2 = False

    if side == 'long' and not cond_vol_base:
        # -------------------------
        # 예외 조건 1
        # -------------------------
        # [-9:-1] 구간에서 음봉 개수 >= 7
        ...
        # 음봉: close < open
        bearish_8 = last_8_ex_prev[
            last_8_ex_prev['close'] < last_8_ex_prev['open']
        ]

        cond_exc1_bearish_count = len(bearish_8) >= 7

        # 몸통 = open - close
        # 꼬리 = close - low
        # 꼬리 / 몸통 >= 1/3
        if len(bearish_8) > 0:
            body_8 = bearish_8['open'] - bearish_8['close']
            tail_8 = bearish_8['close'] - bearish_8['low']

            # body 가 0 이면 division by zero 방지
            ratio_8 = tail_8 / body_8.replace(0, np.nan)
            long_tail_8 = (ratio_8 >= 1.0 / 3.0).sum()
        else:
            long_tail_8 = 0

        cond_exc1_long_tail = long_tail_8 >= 3

        cond_exc1 = (
            cond_exc1_bearish_count
        ...
        if not cond_exc1:
            ...
            # 변동성 = (open - close) / open
            vola_9 = (
                (last_9_incl_prev['open'] - last_9_incl_prev['close'])
                / last_9_incl_prev['open']
            )

            bearish_9 = last_9_incl_prev[
                last_9_incl_prev['close'] < last_9_incl_prev['open']
            ]

            if len(bearish_9) > 0:
                vola_bearish_9 = (
                    (bearish_9['open'] - bearish_9['close'])
                    / bearish_9['open']
                )

                cond_vol_05 = (vola_bearish_9 >= 0.005).sum() >= 2
                cond_vol_035 = (vola_bearish_9 >= 0.0035).sum() >= 3
            else:
                cond_vol_05 = False
                cond_vol_035 = False
            ...
            # 꼬리 / 몸통 >= 1/3 인 캔들 개수
            if len(bearish_9) > 0:
                body_9 = bearish_9['open'] - bearish_9['close']
                tail_9 = bearish_9['close'] - bearish_9['low']

                ratio_9 = tail_9 / body_9.replace(0, np.nan)
                long_tail_9 = (ratio_9 >= 1.0 / 3.0).sum()
            else:
                long_tail_9 = 0

            cond_exc2_long_tail = long_tail_9 >= 2
    ...
    # -------------------------------------------------
    # 6) 15m 전용 예외 조건 (롱/숏 공통)
    # -------------------------------------------------
    cond_15m_common1 = False
    cond_15m_common2 = False
    cond_15m_extra_tail_rule = False
    cond_15m_exc1 = False
    cond_15m_exc2 = False

    if not cond_vol_base:
        # 공통 조건 1: 직전 12 개봉 중 (직전 11 개봉 high/low) 기준
        if side == 'long':
            # ✅ 롱: (직전 11 개봉 low)*1.001 < 직전봉 종가
            min_low_11 = last_11_ex_prev['low'].min()
            cond_common1 = prev['close'] < min_low_11 * 1.001
        else:
            # 숏: (직전 11 개봉 high)*0.999 > 직전봉 종가
            max_high_11 = last_11_ex_prev['high'].max()
            cond_common1 = prev['close'] > max_high_11 * 0.999
        
        # 공통 조건 2: 캔들 변동성 0.1% 이상인데 꼬리가 0.4% 이상
        # ✅ 롱: 음봉의 아래꼬리 (close - low)
        # ✅ 숏: 양봉의 윗꼬리 (high - close)
        # 몸통: abs(open - close)
        
        # 직전 12 개봉 중 변동성 0.1% 이상인 캔들 필터
        candles_12 = last_12_incl_prev
        body_12 = abs(candles_12['open'] - candles_12['close'])
        volatility_12 = body_12 / candles_12['open']
        
        # 변동성 0.1% 이상인 캔들만 필터
        cond_vol_01 = volatility_12 >= 0.001
        
        if side == 'long':
            # ✅ 롱: 음봉만 (close < open)
            cond_bearish = candles_12['close'] < candles_12['open']
            # ✅ 롱: 아래꼬리 = close - low
            tail_12 = candles_12['close'] - candles_12['low']
        else:
            # ✅ 숏: 양봉만 (close > open)
            cond_bearish = candles_12['close'] > candles_12['open']
            # ✅ 숏: 윗꼬리 = high - close
            tail_12 = candles_12['high'] - candles_12['close']
        
        tail_ratio_12 = tail_12 / body_12.replace(0, np.nan)
        cond_tail_04 = tail_ratio_12 >= (0.004 / candles_12['open'])  # 0.4% 절대값
        
        # 꼬리 2 개 또는 0.4% 캔들 1 개로 취급
        # 꼬리 개수: 변동성 0.1% 이상 + 꼬리 0.4% 이상
        tail_count = (cond_vol_01 & cond_tail_04 & cond_bearish).sum()
        
        # 0.4% 캔들 1 개: 변동성 0.1% 이상 + 꼬리 0.4% 이상인 캔들 중 1 개
        cond_common2 = tail_count >= 2 or (tail_count >= 1 and (cond_vol_01 & cond_tail_04 & cond_bearish).any())
        
        # 공통 조건 1 AND 공통 조건 2
        cond_common = cond_common1 and cond_common2
        
        if cond_common:
            # ✅ 롱: 음봉 변동성, 숏: 양봉 변동성
            if side == 'long':
                # ✅ 롱: 음봉만 (close < open)
                bearish_candles_12 = candles_12[candles_12['close'] < candles_12['open']]
                if len(bearish_candles_12) > 0:
                    vola_bearish_12 = (
                        (bearish_candles_12['open'] - bearish_candles_12['close'])
                        / bearish_candles_12['open']
                    )
                    body_bearish_12 = bearish_candles_12['open'] - bearish_candles_12['close']
                    tail_bearish_12 = bearish_candles_12['close'] - bearish_candles_12['low']
                    tail_ratio_bearish_12 = tail_bearish_12 / body_bearish_12.replace(0, np.nan)
                else:
                    vola_bearish_12 = pd.Series(dtype='float64')
                    tail_ratio_bearish_12 = pd.Series(dtype='float64')
            else:
                # ✅ 숏: 양봉만 (close > open)
                bullish_candles_12 = candles_12[candles_12['close'] > candles_12['open']]
                if len(bullish_candles_12) > 0:
                    vola_bullish_12 = (
                        (bullish_candles_12['close'] - bullish_candles_12['open'])
                        / bullish_candles_12['open']
                    )
                    body_bullish_12 = bullish_candles_12['close'] - bullish_candles_12['open']
                    tail_bullish_12 = bullish_candles_12['high'] - bullish_candles_12['close']
                    tail_ratio_bullish_12 = tail_bullish_12 / body_bullish_12.replace(0, np.nan)
                else:
                    vola_bullish_12 = pd.Series(dtype='float64')
                    tail_ratio_bullish_12 = pd.Series(dtype='float64')
            
            if side == 'long':
                # 진입 예외 조건 3: 캔들 변동성 0.3% 이상 3 개, 0.4% 이상 1 개 + 꼬리 2 개 이상
                cond_vol_03 = (vola_bearish_12 >= 0.003).sum() >= 3
                cond_vol_04 = (vola_bearish_12 >= 0.004).sum() >= 1
                # ✅ 꼬리 1/3 이상인 캔들만 카운트 (변동성 0.3% 이상 또는 0.4% 이상인 캔들 중)
                cond_vol_03_04 = (vola_bearish_12 >= 0.003) | (vola_bearish_12 >= 0.004)
                cond_tail_1_3 = (tail_ratio_bearish_12 >= 1.0 / 3.0) & cond_vol_03_04
                cond_tail_2 = cond_tail_1_3.sum() >= 2
                
                cond_exc3 = cond_vol_03 and cond_vol_04 and cond_tail_2
                
                # 진입 예외 조건 4: 0.2% 3 개 이상, 0.5% 2 개 이상 + 꼬리 2 개 이상
                cond_vol_02 = (vola_bearish_12 >= 0.002).sum() >= 3
                cond_vol_05 = (vola_bearish_12 >= 0.005).sum() >= 2
                # ✅ 꼬리 1/3 이상인 캔들만 카운트 (변동성 0.2% 이상 또는 0.5% 이상인 캔들 중)
                cond_vol_02_05 = (vola_bearish_12 >= 0.002) | (vola_bearish_12 >= 0.005)
                cond_tail_1_3_v2 = (tail_ratio_bearish_12 >= 1.0 / 3.0) & cond_vol_02_05
                cond_tail_2_v2 = cond_tail_1_3_v2.sum() >= 2
                
                cond_exc4 = cond_vol_02 and cond_vol_05 and cond_tail_2_v2
                
                # 진입 예외 조건 5: 0.5% 3 개이상 + 0.4% 4 개이상 이면 꼬리 1 개로 진입 가능
                cond_vol_05_v2 = (vola_bearish_12 >= 0.005).sum() >= 3
                cond_vol_04_v2 = (vola_bearish_12 >= 0.004).sum() >= 4
                # ✅ 꼬리 1/3 이상인 캔들만 카운트 (변동성 0.5% 이상 또는 0.4% 이상인 캔들 중)
                cond_vol_05_04 = (vola_bearish_12 >= 0.005) | (vola_bearish_12 >= 0.004)
                cond_tail_1_3_v3 = (tail_ratio_bearish_12 >= 1.0 / 3.0) & cond_vol_05_04
                cond_tail_1 = cond_tail_1_3_v3.sum() >= 1
                
                cond_exc5 = cond_vol_05_v2 and cond_vol_04_v2 and cond_tail_1
            else:
                # ✅ 숏: 양봉 변동성
                cond_vol_03 = (vola_bullish_12 >= 0.003).sum() >= 3
                cond_vol_04 = (vola_bullish_12 >= 0.004).sum() >= 1
                # ✅ 꼬리 1/3 이상인 캔들만 카운트 (변동성 0.3% 이상 또는 0.4% 이상인 캔들 중)
                cond_vol_03_04 = (vola_bullish_12 >= 0.003) | (vola_bullish_12 >= 0.004)
                cond_tail_1_3 = (tail_ratio_bullish_12 >= 1.0 / 3.0) & cond_vol_03_04
                cond_tail_2 = cond_tail_1_3.sum() >= 2
                
                cond_exc3 = cond_vol_03 and cond_vol_04 and cond_tail_2
                
                cond_vol_02 = (vola_bullish_12 >= 0.002).sum() >= 3
                cond_vol_05 = (vola_bullish_12 >= 0.005).sum() >= 2
                # ✅ 꼬리 1/3 이상인 캔들만 카운트 (변동성 0.2% 이상 또는 0.5% 이상인 캔들 중)
                cond_vol_02_05 = (vola_bullish_12 >= 0.002) | (vola_bullish_12 >= 0.005)
                cond_tail_1_3_v2 = (tail_ratio_bullish_12 >= 1.0 / 3.0) & cond_vol_02_05
                cond_tail_2_v2 = cond_tail_1_3_v2.sum() >= 2
                
                cond_exc4 = cond_vol_02 and cond_vol_05 and cond_tail_2_v2
                
                cond_vol_05_v2 = (vola_bullish_12 >= 0.005).sum() >= 3
                cond_vol_04_v2 = (vola_bullish_12 >= 0.004).sum() >= 4
                # ✅ 꼬리 1/3 이상인 캔들만 카운트 (변동성 0.5% 이상 또는 0.4% 이상인 캔들 중)
                cond_vol_05_04 = (vola_bullish_12 >= 0.005) | (vola_bullish_12 >= 0.004)
                cond_tail_1_3_v3 = (tail_ratio_bullish_12 >= 1.0 / 3.0) & cond_vol_05_04
                cond_tail_1 = cond_tail_1_3_v3.sum() >= 1
                
                cond_exc5 = cond_vol_05_v2 and cond_vol_04_v2 and cond_tail_1
            
            # ✅ 예외 조건 3, 4, 5 중 하나라도 만족하면 거래량 조건 없이 바로 진입
            if cond_exc3 or cond_exc4 or cond_exc5:
                cond_vol_base = True
        
        # -------------------------------------------------
        # 15m 전용 예외 조건
        # -------------------------------------------------
        if timeframe == '15m' and cond_common:
            # 15m 공통 조건 1
            cond_15m_common1 = cond_common1
            
            # 15m 공통 조건 2: 캔들 변동성 0.1% 이상인데 꼬리가 0.4% 이상
            # 꼬리 2 개 또는 0.4% 캔들 1 개로 취급
            cond_15m_common2 = cond_common2
            
            # 추가 규칙: 0.15% 3 개 이상이면 1/3 꼬리 1 개, 0.4% 봉 1 개로 취급
            if side == 'long':
                tail_on_015 = (vola_bearish_12 >= 0.0015) & (tail_ratio_bearish_12 >= 1.0 / 3.0)
                cond_15m_extra_tail_rule = (
                    (vola_bearish_12 >= 0.0015).sum() >= 3
                    and tail_on_015.sum() >= 1
                )
            else:
                tail_on_015 = (vola_bullish_12 >= 0.0015) & (tail_ratio_bullish_12 >= 1.0 / 3.0)
                cond_15m_extra_tail_rule = (
                    (vola_bullish_12 >= 0.0015).sum() >= 3
                    and tail_on_015.sum() >= 1
                )
            
            # 15m 예외 1: 0.4% 3 개 이상, 꼬리 2 개 이상
            if side == 'long':
                candles_vol_04 = vola_bearish_12 >= 0.004
                tail_on_04 = candles_vol_04 & (tail_ratio_bearish_12 >= 1.0 / 3.0)
                cond_15m_exc1 = (
                    (vola_bearish_12 >= 0.004).sum() >= 3
                    and tail_on_04.sum() >= 2
                )
            else:
                candles_vol_04 = vola_bullish_12 >= 0.004
                tail_on_04 = candles_vol_04 & (tail_ratio_bullish_12 >= 1.0 / 3.0)
                cond_15m_exc1 = (
                    (vola_bullish_12 >= 0.004).sum() >= 3
                    and tail_on_04.sum() >= 2
                )
            
            # 15m 예외 2: 0.15% 4 개 이상, 0.3% 2 개 이상, 꼬리 2 개 이상
            if side == 'long':
                candles_vol_015 = vola_bearish_12 >= 0.0015
                candles_vol_03 = vola_bearish_12 >= 0.003
                tail_on_015 = candles_vol_015 & (tail_ratio_bearish_12 >= 1.0 / 3.0)
                cond_15m_exc2 = (
                    candles_vol_015.sum() >= 4
                    and candles_vol_03.sum() >= 2
                    and tail_on_015.sum() >= 2
                )
            else:
                candles_vol_015 = vola_bullish_12 >= 0.0015
                candles_vol_03 = vola_bullish_12 >= 0.003
                tail_on_015 = candles_vol_015 & (tail_ratio_bullish_12 >= 1.0 / 3.0)
                cond_15m_exc2 = (
                    candles_vol_015.sum() >= 4
                    and candles_vol_03.sum() >= 2
                    and tail_on_015.sum() >= 2
            ...
            # 15m 예외 1 또는 2 는 거래량 조건 없이 진입 가능
            if cond_15m_exc1 or cond_15m_exc2:
                cond_vol_base = True
    ...
    return {
        ...
        "range_volatility": float(range_volatility),
        "range_vol_threshold": float(range_vol_threshold),
        ...
        "vol_mult_required": (
            vol_mult_base
            if cond_vol_base and (side == 'short' or (cond_exc1 is False and cond_exc2 is False and cond_exc3 is False and cond_exc4 is False and cond_exc5 is False))
            else (
                vol_mult_exc1 if cond_exc1 else (vol_mult_exc2 if cond_exc2 else 2.0)
            )
        ),
        ...
        # 롱 전용 예외 조건 정보
        "cond_exc1": cond_exc1 if side == 'long' else None,
        "cond_exc2": cond_exc2 if side == 'long' else None,
        
        # 롱/숏 공통 예외 조건 정보
        "cond_exc3": cond_exc3,
        "cond_exc4": cond_exc4,
        "cond_exc5": cond_exc5,
        "cond_common1": cond_common1 if (cond_exc3 or cond_exc4 or cond_exc5) else None,
        "cond_common2": cond_common2 if (cond_exc3 or cond_exc4 or cond_exc5) else None,
        
        # 15m 전용 예외 조건 정보
        "cond_15m_common1": cond_15m_common1,
        "cond_15m_common2": cond_15m_common2,
        "cond_15m_extra_tail_rule": cond_15m_extra_tail_rule,
        "cond_15m_exc1": cond_15m_exc1,
        "cond_15m_exc2": cond_15m_exc2,
...
def trade_rsi_close_strategy_link(
    ...
    global last_link_long_15m, last_link_short_15m
    ...
    if side == 'long':
        if now - last_link_long_trade_time < 60:
            ...
            return
        if timeframe == '5m' and now - last_link_long_5m < 400:
            minutes_ago = (now - last_link_long_5m) / 60
            print(f"[{symbol} LINK_LONG 5m] 최근 {minutes_ago:.1f}분 전에 매수됨 (6 분 내 중복매수 금지)")
            return
        if timeframe == '15m' and now - last_link_long_15m < 900:
            minutes_ago = (now - last_link_long_15m) / 60
            print(f"[{symbol} LINK_LONG 15m] 최근 {minutes_ago:.1f}분 전에 매수됨 (15 분 내 중복매수 금지)")
            return
    else:
        if now - last_link_short_trade_time < 60:
            print(f"[{symbol} LINK_SHORT] 60 초 쿨다운 중 진입 금지")
            return
        if timeframe == '5m' and now - last_link_short_5m < 400:
            minutes_ago = (now - last_link_short_5m) / 60
            print(f"[{symbol} LINK_SHORT 5m] 최근 {minutes_ago:.1f}분 전에 매도됨 (6 분 내 중복매도 금지)")
            return
        if timeframe == '15m' and now - last_link_short_15m < 900:
            minutes_ago = (now - last_link_short_15m) / 60
            print(f"[{symbol} LINK_SHORT 15m] 최근 {minutes_ago:.1f}분 전에 매도됨 (15 분 내 중복매도 금지)")
            return
    ...
    if side == 'long':
        ...
        print(f"[{symbol} LINK_LONG] VOLUME_SPIKE={vol_signal}")
        ...
        sl_price = vol_signal["prev_close"] * (1 - 0.035)
    ...
    # -------------------------------------------------
        ...
        print(f"[{symbol} LINK_SHORT] VOLUME_SPIKE={vol_signal}")
...
def analyze_bearish_divergence(
    ...
    # 3~11, 2번 봉 제외
    # 기존 코드의 6개 봉 기준 유지 - 숏은 6개봉으로만 다이버 판단  직전봉 제외 6개봉 직전봉-1, -2 ~ -7(6개봉)
    base_15 = df.iloc[-8:-2]
...
def analyze_bearish_divergence_close(
    ...
    # 숏은 6 봉 기준
    base_15 = df.iloc[-8:-2]  # -2,-3,-4,-5,-6,-7 직전봉-1
...
def analyze_bullish_divergence_pistol(
    ...
    return {
        ...
        "volatility_12_strong_condition": (
            cond_volatility_12_strong
        ),
...
def trade_rsi_pistol_strategy(
    ...
    print(
        f"[{symbol} {timeframe} RSI_PISTOL] "
        f"available_usdt={available_usdt:.4f}, "
    ...
    print(
        f"[{symbol} {timeframe}] "
        f"BULL_PISTOL={bull_pistol}"
    )

    if bull_pistol is None:
        print(
            f"[{symbol} {timeframe} RSI_PISTOL] "
            f"분석 결과 없음"
        )
...
def analyze_bullish_divergence_close_current(
    ...
    """
    상승 다이버전스 판단 함수 (current용).
    - df[-2] 를 직전 확정봉으로 사용.
    - df[-1] 를 현재 진행 중인 봉으로 사용.
    - 15봉 및 30봉 조건과 현재봉 조건을 함께 본다.
    """
    ...
    # 15봉 기준: 직전 확정봉 이전 15개
    base_15 = df.iloc[-17:-2]  # 15개
    base_16 = df.iloc[-18:-1]  # 변동성 구간

    # 30봉 기준: 그 앞 30개
    base_30 = df.iloc[-32:-17]
    base_31 = df.iloc[-33:-1]
    ...
    cond_price_15 = current_candle['low'] < lowest_close * (1 - price_diff_pct)
    cond_rsi_15 = current_candle['rsi'] >= lowest_rsi * (1 + rsi_raise_pct)
    cond_volatility_15 = abs(current_candle['close'] - current_candle['open']) / current_candle['open'] >= min_volatility
    ...
    cond_price_30 = current_candle['low'] < lowest_close_30 * (1 - price_diff_pct_30)
    cond_rsi_30 = current_candle['rsi'] >= lowest_rsi_30 * (1 + rsi_raise_pct_30)
    cond_volatility_30 = abs(current_candle['close'] - current_candle['open']) / current_candle['open'] >= min_volatility_30

    signal_30 = cond_price_30 and cond_rsi_30 and cond_volatility_30

    # # 현재봉 조건 (간단 예시: 변동성 + 직전봉 대비 가격/RSI)
    # current_body_vol = abs(current_candle['close'] - current_candle['open']) / current_candle['open']
    # current_cond_vol = current_body_vol >= min_volatility
    # current_cond_price = current_candle['close'] > prev_candle['close'] * (1 + price_diff_pct)
    # current_cond_rsi = current_candle['rsi'] >= prev_candle['rsi'] * (1 + rsi_raise_pct)
...
def analyze_bearish_divergence_close_current(
    symbol,
    timeframe,
    rsi_drop_pct=0.02,
    min_volatility=0.003,
    ...
    """
    하락 다이버전스 판단 함수 (current용).
    - df[-2] 를 직전 확정봉으로 사용.
    - df[-1] 를 현재 진행 중인 봉으로 사용.
    - 15봉 조건 + 현재봉 조건을 함께 본다.
    """
    ...
    cond_price_15 = current_candle['high'] > highest_close * (1 + price_diff_pct)
    cond_rsi_15 = current_candle['rsi'] <= highest_rsi * (1 - rsi_drop_pct)
    cond_volatility_15 = abs(current_candle['close'] - current_candle['open']) / current_candle['open'] >= min_volatility

    signal_15 = cond_price_15 and cond_rsi_15 and cond_volatility_15

    # # 현재봉 조건 (예: 직전봉 대비 위쪽으로 더 튀었는지, 변동성)
    # current_body_vol = abs(current_candle['close'] - current_candle['open']) / current_candle['open']
    # current_cond_vol = current_body_vol >= min_volatility
    # current_cond_price = current_candle['close'] > prev_candle['close'] * (1 + price_diff_pct)
    # current_cond_rsi = True  # 필요시 현재봉 RSI 조건 추가
...
def analyze_bullish_divergence_close_new(symbol, timeframe, df_cache, min_volatility=0.003):
    ...
    prev = df.iloc[-1]
    prev2 = df.iloc[-2]  # 2 번째 이전 봉
    prev3 = df.iloc[-3]  # ✅ 3 번째 이전 봉 추가
    base_7 = df.iloc[-8:-2]  # 2 번째봉 제외 이전 3 번째 봉부터 7 개 구간을 보는 형태
    base_16 = df.iloc[-17:-1]  # 2~17, 2 번~16 까지 최초봉 시가도 포함하기위한 목적 봉 포함
    ...
    # 기존 상승 다이버전스 기본 조건
    # 직전봉 RSI 가 기준 최저 RSI 의 90% 보다 큰지 확인
    ...
    last_11 = df.iloc[-11:]
    last_21 = df.iloc[-16:]  # 직전 15 개봉만 정배열 조건 만족하면 되게 하기위해 21 -> 16 으로 수정
    ...
    # 상승장이면 last11 을 쓰고 아니면 21 을 쓴다
    cond_ma = is_bull_stack(last_11) if strong_ma18 else is_bull_stack(last_21)

    # 직전봉 몸통의 아래 경계
    # lower = min(prev['open'], prev['close'])  종가 close 기준보다 low 터치하는 기준도 포함 목적 아래 코드로 변경-> 다시 몸통으로 수정 26/08/24
    lower = prev['close']
    # 직전봉 몸통의 위 경계    
    upper = prev['open']
    ...
    cond_prev2_open_above = (
        (prev2['open'] > prev2['ma50']) or
        (prev2['open'] > prev2['vwma100'])
    )
    
    cond_prev3_open_above = (
        (prev3['open'] > prev3['ma50']) or
        (prev3['open'] > prev3['vwma100'])
    )
    
    # ✅ 두 조건 모두 만족해야 함
    cond_prev2_prev3 = cond_prev2_open_above and cond_prev3_open_above

    # 기존 다이버전스 or 정배열 RSI+ 중 하나라도 맞으면 진입 후보
    # 단, 변동성/정배열/터치 조건도 모두 함께 확인
        
    # ✅ 기존 signal 에 cond_prev2_prev3 추가
    signal = cond_div and cond_vol and cond_ma and cond_touch and cond_prev2_prev3
    ...
    return {
        ...
        "prev3_open": float(prev3['open']),  # ✅ 추가
        "prev3_close": float(prev3['close']),  # ✅ 추가
        "prev3_rsi": float(prev3['rsi']),  # ✅ 추가
        "lowest_rsi": float(lowest_rsi),
        "lowest_close": float(lowest_close),
        ...
        "prev2_prev3_condition": cond_prev2_prev3,  # ✅ 추가
        "tp_price": float(prev['close'])
    }    
    
#3 eth 전략 ### 이평선 전략 추가 기준 전용 - eth 추매 조건은 삭제. 함수 이름 변경 필요
def trade_rsi_close_strategy_eth_long_new(
    ...
    # 1시간봉 전용 180분 쿨다운
    if timeframe == '1h' and now - last_eth_long_1h < 18000:
        minutes_ago = (now - last_eth_long_1h) / 60
        print(f"[{symbol} ETH_LONG_NEW 1h] 최근 {minutes_ago:.1f}분 전에 1 시간봉 매수됨 (300 분 내 중복매수 금지)")
        return

    # 15분봉 전용 45분 쿨다운
    if timeframe == '15m' and now - last_eth_long_15m < 7200:
        minutes_ago = (now - last_eth_long_15m) / 60
        print(f"[{symbol} ETH_LONG_NEW 15m] 최근 {minutes_ago:.1f}분 전에 15 분봉 매수됨 (120 분 내 중복매수 금지)")
        return
    ...
    if bull["range_volatility"] < 0.015: # 직전 15봉 변동성 1.5% 미만시 진입 안함
...
def analyze_50ma_close_strategy(symbol, timeframe, df_cache):
    ...
    if timeframe == '5m':
        ...
        cond_prev1 = prev_close1 >= ma50
        cond_prev2 = prev_close2 < ma50_2        
        cond_range_1 = ((prev_close1 - low_20) / prev_close1) < 0.015  # 50 100 200 완전 역배열  직전 -1.5% 까지 진입
        cond_range_2 = ((prev_close1 - low_20) / prev_close1) < 0.008  # 50 200 100 불완전 역배열 
...
# TP는 전략함수에서 구동중 이건 예전에 남은거라 안쓴다고 보면됨
    ...
    if timeframe == '15m':
        ...
        cond_prev1 = prev_close1 >= ma50
        cond_prev2 = prev_close2 < ma50_2        
        cond_range_1 = ((prev_close1 - low_20) / prev_close1) < 0.025 # 진입시  저가 대비 진입가 변동성  # 50 100 200 완전 역배열
        cond_range_2 = ((prev_close1 - low_20) / prev_close1) < 0.008  # 50 200 100 불완전 역배열

        case1_stack = ma50 < vwma100 < ma200 # 완전 역배열
        case2_stack = ma50 < ma200 < vwma100 # 불완전 역배열
    ...
    if timeframe == '1h':
        if len(df) < 50:  # 20,25,50,200,100 다 돌려면 충분히 필요
            return None
        ...
        # 조건 2: abs((ma50 - vwma100) / ma50) > 0.3%
        cond_ma50_vwma = abs(ma50 - vwma100) / ma50 > 0.003

        # 조건 3: abs((ma20 - ma25) / ma20) < 0.3%
        cond_ma20_25 = abs(ma20 - ma25) / ma20 < 0.003

        # 조건 4: 직전봉 close > ma20, ma25
        cond_close_above_20_25 = (prev_close > ma20) and (prev_close > ma25)

        # 조건 5: ma50 < ma200
        cond_ma50_under_200 = ma50 < ma200
        ...
        return {
            ...
            "vwma100": vwma100,
            "cond_vol": cond_vol,
            "cond_ma50_vwma": cond_ma50_vwma,
            "cond_ma20_25": cond_ma20_25,
...
def trade_50ma_close_strategy(symbol, market_id, timeframe):
    ...
    # RSI + 이평 데이터
    # ✅ 21 → 42 로 수정 (MA50, MA200, VWMA100 계산용)
    df = get_confirmed_candles_with_rsi_new(symbol, timeframe)
    if df is None or len(df) < 42:
        print(f"[{symbol} 50MA_CLOSE] 확정봉 데이터 부족 (최소 42 개 필요)")
        return
    ...
    if timeframe == '5m':
        ...
        # 1h 등 기타는 기존 로직 사용
        if sig_info.get("tp_pct_base") is not None:
...
def trade_rsi_close_strategy_current(
    ...
    """
    현재봉까지 고려하는 close 기준 RSI 다이버전스 전략 (current용).
    기존 trade_rsi_close_strategy 와 병렬로 사용할 수 있다.
    """
    ...
    if now - last_sol_trade_time < 60:
        print(f"[{symbol} {timeframe} RSI_CLOSE_CURRENT] 60 초 쿨다운 중 진입 금지 (지난 체결 후 {now - last_sol_trade_time:.1f}초 경과)")
        return

    if timeframe == '1h' and now - last_sol_buy_time_1h < 10800:
        minutes_ago = (now - last_sol_buy_time_1h) / 60
        print(f"[{symbol} {timeframe} RSI_CLOSE_CURRENT] 최근 {minutes_ago:.1f}분 전에 1 시간봉 매수됨 (180 분 내 중복매수 금지)")
        return

    if timeframe == '15m' and now - last_sol_buy_time_15m < 2700:
        minutes_ago = (now - last_sol_buy_time_15m) / 60
        print(f"[{symbol} {timeframe} RSI_CLOSE_CURRENT] 최근 {minutes_ago:.1f}분 전에 15 분봉 매수됨 (45 분 내 중복매수 금지)")
        return
    ...
    bull_close = analyze_bullish_divergence_close_current(
        symbol=symbol,
        timeframe=timeframe,
        rsi_raise_pct=rsi_raise_pct,
        min_volatility=min_volatility,
    ...
    bear_close = analyze_bearish_divergence_close_current(
        symbol=symbol,
        timeframe=timeframe,
        rsi_drop_pct=rsi_drop_pct,
        min_volatility=min_volatility,
    ...
    print(f"[{symbol} {timeframe}] BULL_CLOSE_CURRENT={bull_close}")
    print(f"[{symbol} {timeframe}] BEAR_CLOSE_CURRENT={bear_close}")
    ...
    if bull_close and bull_close["signal"]:
        ...
        current_close = bull_close["current_close"]
...
last_link_long_15m = 0
last_link_short_15m = 0
...
while True:
    try:
        ...
        # 1시간봉에서 포지션이 생기지 않은 경우에만 15분봉 평가
        if not has_position(MARKET_ID_SOL):
            trade_rsi_pistol_strategy(
                symbol=SOL_SYMBOL,
                market_id=MARKET_ID_SOL,
        ...
        # if not has_position(MARKET_ID_SOL):
        #     trade_rsi_close_strategy_current(
        #         symbol=SOL_SYMBOL,
        #         market_id=MARKET_ID_SOL,
        #         timeframe='1h',
        #         tp_long_pct=0.01,
